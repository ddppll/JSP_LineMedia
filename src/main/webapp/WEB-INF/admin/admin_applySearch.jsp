...
<html>
<head>
   		<title>admin_applySearch.jsp</title>
...
<!-- 블록 페이징처리 시작(BS4 스타일적용) -->
				<div class="container">
					<ul class="pagination justify-content-center">
						...
						<c:if test="${totPage != 0}">
						  <c:if test="${pag != 1}">
						    <li class="page-item"><a href="admin_applySearch.ad?pag=1&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}" title="첫페이지" class="page-link text-secondary">◁◁</a></li>
						  </c:if>
						  <c:if test="${curBlock > 0}">
						    <li class="page-item"><a href="admin_applySearch.ad?pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}" title="이전블록" class="page-link text-secondary">◀</a></li>
						  </c:if>
						  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}">
						    <c:if test="${i == pag && i <= totPage}">
						      <li class="page-item active"><a href='admin_applySearch.ad?pag=${i}&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}' class="page-link text-light bg-secondary border-secondary">${i}</a></li>
						    </c:if>
						    <c:if test="${i != pag && i <= totPage}">
						      <li class="page-item"><a href='admin_applySearch.ad?pag=${i}&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}' class="page-link text-secondary">${i}</a></li>
						    </c:if>
						  </c:forEach>
						  <c:if test="${curBlock < lastBlock}">
						    <li class="page-item"><a href="admin_applySearch.ad?pag=${(curBlock+1)*blockSize + 1}&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}" title="다음블록" class="page-link text-secondary">▶</a>
						  </c:if>
						  <c:if test="${pag != totPage}">
						    <li class="page-item"><a href="admin_applySearch.ad?pag=${totPage}&pageSize=${pageSize}&lately=${lately}&search=${search}&searchString=${searchString}" title="마지막페이지" class="page-link" style="color:#555">▷▷</a>
						  </c:if>
						</c:if>
					</ul>
				</div>
